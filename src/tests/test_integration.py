...
class TestPipelineIntegration:
    ...
    def test_full_pipeline_small_dataset(
        self, mock_spark, mock_load_dataset, pipeline_config, sample_batch_data
    ):
        ...
        mock_spark.builder.appName.return_value.config.return_value.getOrCreate.return_value = (
            mock_spark_session
        )
    ...
    @patch("pipeline.amazon_pipeline.load_dataset")
    @patch("pipeline.amazon_pipeline.SparkSession")
    def test_pipeline_with_processing_errors(
        self, mock_spark, mock_load_dataset, pipeline_config
    ):
    ...
    @patch("pipeline.amazon_pipeline.load_dataset")
    @patch("pipeline.amazon_pipeline.SparkSession")
    def test_pipeline_bigquery_failure_handling(
        self, mock_spark, mock_load_dataset, pipeline_config, sample_batch_data
    ):
    ...
    def test_batch_processing_logic(self, pipeline_config):
        ...
        for i in range(25):  # 25 records
            large_dataset.append(
                {"title": f"Product {i}", "main_cat": "Beauty", "average_rating": 4.0}
            )
