...
class AmazonReviewsPipeline:
    ...
    def _create_spark_session(self) -> SparkSession:
        ...
        spark = SparkSession.builder \
            .appName("Amazon-Reviews-to-BigQuery") \
    ...
    def process_batch_data(self, batch_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        for record in batch_data:
            try:
                ...
                for key, value in record.items():
                    if key in ['features', 'categories', 'images', 'videos', 'description', 'bought_together']:
                        # Keep arrays as arrays for BigQuery
                        processed_record[key] = value if isinstance(value, list) else []
                    elif isinstance(value, (dict, list)) and key not in ['features', 'categories', 'images', 'videos', 'description', 'bought_together']:
                        ...
                        processed_record[key] = float(value) if value is not None else None
                    elif key in ['rating_number']:
                        processed_record[key] = int(value) if value is not None else None
                    else:
    ...
    def write_to_bigquery(self, df, write_mode: str = "append"):
        ...
        try:
            df.write \
                ...
                .option("table", self.table_full_name) \
                .option("writeDisposition", "WRITE_APPEND" if write_mode == "append" else "WRITE_TRUNCATE") \
    ...
    def run_pipeline(self, batch_size: int = 10000, max_batches: int = None):
        ...
        logger.info(f"Success rate: {((batch_count-failed_batches)/batch_count*100):.1f}%")
    ...
    def _process_single_batch(self, batch_data: List[Dict], batch_num: int, total_so_far: int) -> bool:
        ...
        try:
            ...
            try:
                with open(f"failed_batch_{batch_num}_{int(time.time())}.json", "w") as f:
                    json.dump(batch_data[:5], f, indent=2, default=str)  # Save first 5 records
...
def main():
    ...
    parser.add_argument("--dataset-id", default="amazon_reviews", help="BigQuery dataset ID")
    parser.add_argument("--table-id", default="beauty_metadata", help="BigQuery table ID")
